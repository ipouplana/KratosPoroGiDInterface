proc WritePoroMaterials { basename dir problemtypedir PropertyId } {
    ...
    for {set i 0} {$i < [llength $Groups]} {incr i} {
        ...
        puts $FileVar "            \},"
        puts $FileVar "            \"Variables\": \{"
        puts $FileVar "                \"NORMAL_STIFFNESS\": [lindex [lindex $Groups $i] 5],"
        puts $FileVar "                \"SHEAR_STIFFNESS\": [lindex [lindex $Groups $i] 6],"
        puts $FileVar "                \"PENALTY_STIFFNESS\": [lindex [lindex $Groups $i] 7],"
        puts $FileVar "                \"YOUNG_MODULUS\": [lindex [lindex $Groups $i] 8],"
        puts $FileVar "                \"POISSON_RATIO\": [lindex [lindex $Groups $i] 9],"
        puts $FileVar "                \"DENSITY_SOLID\": [lindex [lindex $Groups $i] 10],"
        puts $FileVar "                \"DENSITY_WATER\": [lindex [lindex $Groups $i] 11],"
        puts $FileVar "                \"POROSITY\": [lindex [lindex $Groups $i] 12],"
        puts $FileVar "                \"BULK_MODULUS_SOLID\": [lindex [lindex $Groups $i] 13],"
        puts $FileVar "                \"BULK_MODULUS_FLUID\": [lindex [lindex $Groups $i] 14],"
        puts $FileVar "                \"TRANSVERSAL_PERMEABILITY\": [lindex [lindex $Groups $i] 15],"
        puts $FileVar "                \"DYNAMIC_VISCOSITY\": [lindex [lindex $Groups $i] 16],"
        puts $FileVar "                \"THICKNESS\": [lindex [lindex $Groups $i] 17],"
        puts $FileVar "                \"DAMAGE_THRESHOLD\": [lindex [lindex $Groups $i] 18],"
        puts $FileVar "                \"MINIMUM_JOINT_WIDTH\": [lindex [lindex $Groups $i] 19],"
        puts $FileVar "                \"CRITICAL_DISPLACEMENT\": [lindex [lindex $Groups $i] 20],"
        puts $FileVar "                \"YIELD_STRESS\": [lindex [lindex $Groups $i] 21],"
        puts $FileVar "                \"FRICTION_COEFFICIENT\": [lindex [lindex $Groups $i] 22],"
        puts $FileVar "                \"FRACTURE_ENERGY\": [lindex [lindex $Groups $i] 25],"
        puts $FileVar "                \"SHEAR_FRACTURE_ENERGY\": [lindex [lindex $Groups $i] 26],"
        puts $FileVar "                \"STATE_VARIABLE\": [lindex [lindex $Groups $i] 27],"
